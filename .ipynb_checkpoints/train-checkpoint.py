...
def process_data(eqname, zero_shot, alpha_list, beta_list, i_size, b_size, f_size, low, high, batch_count):
    i_set = []
    b_set = []
    f_set = []
    ...
    return loader_train_initial, loader_train_boundary, loader_train_domain, zero_shot_num
...
def train(epochs=1000, lr=0.1, i_size=500, b_size=500, f_size=1000, load=False, zero_shot=False, alpha_list=None, beta_list=None, load_data=None, low=-1, high=1, eqname='poisson'):
    ...
    loader_train_initial, loader_train_boundary, loader_train_domain, zero_shot_num = process_data(eqname, zero_shot, alpha_list, beta_list, i_size, b_size, f_size, low, high, batch_count)
    ...
    if eqname == 'burgers':
        ...
        alpha_val_ood = np.random.uniform(low=0.1/np.pi, high=0.2/np.pi, size=5)
        ...
        for s in range(5):
            p = np.random.rand()
            if p < 0.5:
                alpha_val_ood[s] = np.random.uniform(low=0.001/np.pi, high=0.005/np.pi, size=1)[0]
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 99 and zero_shot:
                ...
                loader_val_initial, loader_val_boundary, loader_val_domain, _ = process_data(eqname, zero_shot, alpha_val, beta_list, i_size, b_size, f_size, low, high, batch_count)
                loader_val_ood_initial, loader_val_ood_boundary, loader_val_ood_domain, _ = process_data(eqname, zero_shot, alpha_val_ood, beta_list, i_size, b_size, f_size, low, high, batch_count)

                for data_i, data_b, data_f in list(zip(loader_val_initial, loader_val_boundary, loader_val_domain)):      
                    model.eval()
                    input_i, target_i = data_i
                    input_b, target_b = data_b
                    input_f, target_f = data_f
                ...
                print("Validation | Loss_I: {1:.4f} | Loss_B: {2:.4f} | Loss_F: {3:.4f}".format(epoch + 1, np.mean(val_loss_i), np.mean(val_loss_b), np.mean(val_loss_f)))

                for data_i, data_b, data_f in list(zip(loader_val_ood_initial, loader_val_ood_boundary, loader_val_ood_domain)):      
                    model.eval()
                    input_i, target_i = data_i
                    input_b, target_b = data_b
                    input_f, target_f = data_f
                    ...
                    loss_f = model.calc_loss_f_burgers(input_f, target_f) if zero_shot else model.calc_loss_f_burgers(input_f, target_f, alpha=alpha)
                    loss = loss_i + loss_b + loss_f
                    val_ood_loss_i += [loss_i.item()]
                    val_ood_loss_b += [loss_b.item()]
                    val_ood_loss_f += [loss_f.item()]
                    val_ood_loss_total  += [loss.item()]

                print("Validation OOD | Loss_I: {1:.4f} | Loss_B: {2:.4f} | Loss_F: {3:.4f}".format(epoch + 1, np.mean(val_ood_loss_i), np.mean(val_ood_loss_b), np.mean(val_ood_loss_f)))
        ...
        alpha_val, beta_val = sample_data(100, -1, 1), sample_data(100, -1, 1)
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 99:
                if zero_shot:
                    ...
                    for data_b, data_f in list(zip(loader_val_boundary, loader_val_domain)):      
                        model.eval()
                        input_b, target_b = data_b
                        input_f, target_f = data_f
                    ...
                    print("Validation | Loss_I: {1:.4f} | Loss_B: {2:.4f} | Loss_F: {3:.4f}".format(epoch + 1, np.nan, np.mean(val_loss_b), np.mean(val_loss_f)))

                    for data_b, data_f in list(zip(loader_val_ood_boundary, loader_val_ood_domain)):      
                        model.eval()
                        input_b, target_b = data_b
                        input_f, target_f = data_f
                    ...
                    print("Validation OOD | Loss_I: {1:.4f} | Loss_B: {2:.4f} | Loss_F: {3:.4f}".format(epoch + 1, np.nan, np.mean(val_ood_loss_b), np.mean(val_ood_loss_f)))
                    with torch.no_grad():
                        model.eval()
            ...
            if epoch % 1000 == 999:
                with torch.no_grad():
                    fname = 'models/poisson_zs_high{}_{}.data'.format(high, epoch + 1) if zero_shot else 'models/poisson_high{}_{}.data'.format(high, epoch + 1)
    ...
    val_ood_losses = {
                    'loss_i': val_ood_loss_i,
                    'loss_b': val_ood_loss_b,
                    'loss_f': val_ood_loss_f,
                    'loss': val_ood_loss_total
    } 

    return train_loss_i, train_loss_b, train_loss_f, train_loss, model, val_loss, val_ood_loss, nrmse_set, val_losses, val_ood_losses
