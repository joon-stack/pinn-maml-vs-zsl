...
def make_training_initial_data(i_size, zero_shot=False, z=None):
    ...
    if zero_shot:
        z_i = make_tensor(np.full((i_size, 1), z))
        return [torch.cat([x_i, t_i, z_i], axis=1), u_i]
...
def make_training_boundary_data(b_size, zero_shot=False, z=None):
    ...
    if zero_shot:
        z_b = make_tensor(np.full((b_size, 1), z))
...
def make_training_domain_data(f_size, zero_shot=False, z=None):
    ...
    if zero_shot:
        z_f = make_tensor(np.full((f_size, 1), z))
        return [torch.cat([x_f, t_f, z_f], axis=1), u_f]
